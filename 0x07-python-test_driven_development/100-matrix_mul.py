...
def matrix_mul(m_a, m_b):
    ...
    Raises:
        ...
        TypeError: If m_a or m_b is not a list of lists, if m_a or m_b is empty, or if elements are not integers or floats.
    ...
    if any(len(row) != len(m_a[0]) for row in m_a) or any(len(row) != len(m_b[0]) for row in m_b):
        raise TypeError("each row of m_a must be of the same size" if len(set(len(row) for row in m_a)) != 1 else "each row of m_b must be of the same size")
